...
const app=fs.readFileSync('app/src/App.tsx','utf8');const sections=fs.readFileSync('app/src/components/VideoSections.tsx','utf8');const hook=fs.readFileSync('app/src/hooks/useVideos.ts','utf8');const workflow=fs.readFileSync('app/src/constants/video-workflow.ts','utf8');const config=fs.readFileSync('vite.config.ts','utf8');const pkg=JSON.parse(fs.readFileSync('package.json','utf8'));
describe('Supabase React integration',()=>{
  ...
  it('keeps loading, retry, and section empty states',()=>{expect(app).toContain('Could not load videos. Please try again.');expect(app).toContain('videos.load()');expect(sections).toContain('No active videos right now.');expect(sections).toContain('Completed videos will appear here.');});
